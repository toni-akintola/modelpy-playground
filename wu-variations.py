...
class EpistemicAdvantageModel:
    def __init__(self):
        ...
        # Define Parameters
        self.num_agents = 40  # number of nodes in network, alternates between 3, 6, 12, 18
    ...
    def initialize_graph(self):
        if self.model_type == "variation1":
            self.graph = nx.graph(self.num_agents)
        else:
        ...
        for node in self.graph.nodes():
            initial_data = {
                ...
                'type': 'dominant' if random.random() > self.proportion_marginalized else 'marginalized'
            }
        ...
        if self.model_type == "variation1":
            for potential_neighbor in self.graph.nodes():
                ...
                elif self.graph.nodes[potential_neighbor]["type"] != self.graph.nodes[node]["type"]:
    ...
    def timestep(self):
        ...
        for _node, node_data in self.graph.nodes(data=True):
            ...
            # agent pulls the "b" bandit arm
                ...
                node_data['b_evidence'] = int(np.random.binomial(
                    self.num_pulls, self.objective_b, size=None))
        ...
        def calculate_posterior(prior_belief: float, num_evidence: float) -> float:
            ...
            pE_evidence = (pEH_likelihood * prior_belief) + ((1 - self.objective_b)**num_evidence) * \
...
# initialize model
new_model = EpistemicAdvantageModel()
...
# print results
print(new_model.graph.nodes(data=True), new_model.graph.edges(data=True))
